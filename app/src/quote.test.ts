...
describe("splitInstallments", () => {
  ...
  // РЕГРЕС: у попередній реалізації кожен платіж округлювався незалежно,
  // тому сума масиву могла НЕ дорівнювати totalCents — клієнт або
  // недоплатив би, або виконавець недоотримав би різницю в кілька центів.
  // Виправлено (`prompts/add-tests.md`) — тест лишається як реґрес-перевірка.
  it("сума платежів має дорівнювати totalCents навіть при округленні", () => {
    ...
    expect(sum).toBe(total); // У попередній реалізації давало [33, 33, 33] = 99, не 100 (виправлено)
  });
  ...
  it("сума платежів коректна для іншого проблемного випадку", () => {
    ...
    const parts = splitInstallments(total, 3);
    const sum = parts.reduce((a, b) => a + b, 0);
    expect(sum).toBe(total); // У попередній реалізації давало 3333*3 = 9999 (виправлено)
  });
